...
class Simulation:
    def __init__(self, Ti, Tf, Ts, quad, traj, ctrl, wind):
        self.Ts = Ts
        self.quad = quad
        self.ctrl = ctrl
        self.wind = wind
        ...
        self.theta_all = np.zeros([numTimeStep, len(quad.quat.eulerZYX)])
    ...
    def save_result(self, i, t):
        ...
        self.theta_all[i, :] = self.quad.quat.eulerZYX
...
def main():
    ...
    sim = Simulation(Ti, Tf, Ts, quad, traj, ctrl, wind)
    ...
    utils.sameAxisAnimation(t_all, traj.wps, pos_all, theta_all, sDes_traj_all, Ts, quad.params, traj.xyzType)
    plt.show()
