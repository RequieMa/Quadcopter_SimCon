    ...
    def __init__(self, *args):
        ...
        if self.windType == 'SINE' or self.windType == 'RANDOMSINE':
            if self.windType == 'SINE':
                ...
                self.qW1_med  = np.deg2rad(args[2])
                self.qW2_med  = np.deg2rad(args[3])
            elif (self.windType == 'RANDOMSINE'):
                [
                    velW_max, velW_min, # Wind Velocity in m/s
                    qW1_max, qW1_min, # Wind Heading in deg
                    qW2_max, qW2_min # Wind Elevation positive = upwards wind in deg
                ] = args

                # Median values
                self.velW_med = (velW_max - velW_min) * rd.random() + velW_min
                self.qW1_med  = np.deg2rad((qW1_max - qW1_min) * rd.random() + qW1_min)
                self.qW2_med  = np.deg2rad((qW2_max - qW2_min) * rd.random() + qW2_min)
            # Wind Velocity
